...
var { DIRECTIVE_RESTART_NEVER, DIRECTIVE_RESTART_UNTIL_SUCCESS, DIRECTIVE_RESTART_ALWAYS } = require('./constants');
...
function run(key, proc, emitter) {
  ...
  child.on('close', function(code, signal) {
    cons.info(key, proc, "Closed with signal " + signal + " and code " + code);
  });
  ...
  child.on('exit', function(code, signal) {
    if(code === 0) {
      ...
      cons.error(key, proc, "Exited with signal " + signal + " and code " + code);
    }

    if(killallReceived) {
      cons.info(key, proc, "Not restarting process as killall has been received");
      return;
    }
    
    // Handle restart directives
    var shouldRestart = false;

    if (proc.directive === DIRECTIVE_RESTART_NEVER || !proc.directive) {
      emitter.emit('killall', signal || 'SIGINT');
      return;
    } else if (proc.directive === DIRECTIVE_RESTART_UNTIL_SUCCESS) {
      shouldRestart = (code !== 0);
    } else if (proc.directive === DIRECTIVE_RESTART_ALWAYS) {
      shouldRestart = true;
    }
    
    if (shouldRestart) {
      cons.info(key, proc, "Restarting process...");
      setTimeout(function() {
...
function start(procs, requirements, envs, portarg, emitter){
  ...
  for(var key in requirements) {
    ...
    for(var i = 0; i < n; i++) {
      ...
      // Parse directive from the key name
      var actualKey = key;
      var directive = DIRECTIVE_RESTART_NEVER;
      if (key.endsWith('?')) {
        actualKey = key.slice(0, -1);
        directive = DIRECTIVE_RESTART_UNTIL_SUCCESS;
      } else if (key.endsWith('!')) {
        actualKey = key.slice(0, -1);
        directive = DIRECTIVE_RESTART_ALWAYS;
      }
      ...
      p.env.PROCFILED_WORKER_NAME = p.env.PROCFILED_WORKER_NAME || actualKey + "." + (i + 1);

      run(actualKey + "." + (i + 1), p, emitter);
